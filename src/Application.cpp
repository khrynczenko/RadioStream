...
Application::Application(const std::filesystem::path& config_directory_path,
    ...
    : config_(config_directory_path / constants::CONFIG_FILE),
      window_(
          nana::API::make_center(config_.options().window_width, config_.options().window_height),
          nana::appear::decorate<nana::appear::minimize, nana::appear::sizable,
                                 nana::appear::maximize, nana::appear::taskbar>()),
      menubar_(window_),
      ...
      stations_database_controller_(
          std::make_shared<StationsDatabaseController>(states_manager_, context_)),
      config_controller_(std::make_shared<ConfigController>(states_manager_, context_)),
      radio_browser_requester_controller_(
          std::make_shared<RadioBrowserRequesterController>(states_manager_, context_)),
      status_bar_controller_(
          std::make_shared<StatusBarController>(states_manager_, context_, status_)) {
    set_language();
    init_menubar();
    init_status();
    register_states();
...
void Application::init_menubar() {
    ...
    menubar_.at(FILE).append(
        localizer_.get_localized_text("Open URL"), [this](nana::menu::item_proxy&) {
            std::thread thread([&]() {
                std::lock_guard<std::mutex> lock(mutex_);
                nana::inputbox::text url(localizer_.get_localized_text("URL"));
                nana::inputbox inbox(window_,
    ...
    menubar_.at(FILE).append(
        localizer_.get_localized_text("Add station"), [this](nana::menu::item_proxy&) {
            ...
            if (inbox.show(station_name, url, country, language, codec, bitrate, tags)) {
                if (url.value().empty()) {
                    std::string message =
                        context_.localizer_.get_localized_text("URL field must not be empty");
                    ...
                    notify(std::make_any<Station>(station_name.value(), url.value(),
    ...
    menubar_.at(RADIOBROWSER)
        .append(localizer_.get_localized_text("Find stations"), [this](nana::menu::item_proxy&) {
            states_manager_.switch_state(States::ID::Search);
        });

    menubar_.at(TOOLS).append(
        localizer_.get_localized_text("Settings"),
        [this](nana::menu::item_proxy&) { states_manager_.switch_state(States::ID::Tools); });

    menubar_.at(HELP).append(
        localizer_.get_localized_text("About"), [this](nana::menu::item_proxy&) {
            // states_manager_.switch_state(States::ID::About);
            std::string message = context_.localizer_.get_localized_text("RADIO_ABOUT");
