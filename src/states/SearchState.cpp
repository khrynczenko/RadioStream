...
void SearchState::build_interface() {
    search_textbox_.multi_lines(false).line_wrapped(false).tip_string(
        context_.localizer_.get_localized_text("Search phrase"));
    ...
    container_.div(
        ...
        "<search_form min=5% max=8% arrange=[30%,8%,15%,10%,10%,10%] gap=1% "
        "margin=1%>"
