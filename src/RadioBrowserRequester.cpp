...
std::vector<nlohmann::json> RadioBrowserRequester::request_stations(
    ...
    if (language != "Any")
        request_parameters.emplace_back("language", language);
    ...
    auto response =
        send_arbitrary_request(advanced_search_uri, request_parameters);
...
std::string RadioBrowserRequester::correct_json_response(
...
nlohmann::json RadioBrowserRequester::send_arbitrary_request(
...
RadioBrowserRequester::RadioBrowserRequester(
    unsigned short int search_stations_limit)
    ...
    for (const auto& station_json : stations_jsons) {
        if (std::stoi(station_json["lastcheckok"].get<std::string>()) == 1) {
            stations.emplace_back(station_json["name"].get<std::string>(),
                                  ...
                                  station_json["bitrate"].get<std::string>() +
                                      std::string(" kbps"),
