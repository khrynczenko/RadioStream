...
Station::Station(std::string_view name, std::string_view url,
...
nana::listbox::oresolver& operator<<(nana::listbox::oresolver& ores,
    ...
    return ores << station.name_ << station.url_ << station.country_
                << station.language_ << station.codec_ << station.bitrate_
                << station.tags_;
}

nana::listbox::iresolver& operator>>(nana::listbox::iresolver& ires,
                                     Station& station) {
    return ires >> station.name_ >> station.url_ >> station.country_ >>
           station.language_ >> station.codec_ >> station.bitrate_ >>
           station.tags_;
}
