...
StationsDatabase::StationsDatabase(const std::filesystem::path& path_to_db_file)
    : database_("SQLite", path_to_db_file.string()), cached_stations_({}) {
...
void StationsDatabase::create_empty_table_if_does_not_exist() {
    ...
    create << "CREATE TABLE IF NOT EXISTS `stations` ( `id` INTEGER PRIMARY "
              "KEY AUTOINCREMENT, `name` TEXT,"
              " `url` TEXT, `country` TEXT, `language` TEXT, `codec` TEXT, "
              "`bitrate` TEXT, `tags` TEXT)";
    create.execute();
}
