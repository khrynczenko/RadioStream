	...
	case events::Event::SEARCH_STATIONS:
	{
		...
		if (!searched_string.empty())
		{
			...
			for (const auto& listed_station : context.stations_manager.get_default_stations())
			{
				...
				{
					if (listed_station.name == name)
					{
						ui_.stations_listbox.at(0).append(name);
					}
				}
			}
		}
		...
		{
			ui_.stations_listbox.clear(0);
			on_notify(ui_.stations_listbox, context, events::Event::POPULATE_LISTBOX);
		}
	}
	break;
	case events::Event::POPULATE_LISTBOX:
	{
		ui_.stations_listbox.auto_draw(false);
		for (const auto& station : context.stations_manager.get_default_stations())
		{
			ui_.stations_listbox.at(0).append({ station.name });
		}
		ui_.stations_listbox.auto_draw(true);
	}
	break;
	case events::Event::UPDATE_STATION_LABEL:
	{
		if (!ui_.stations_listbox.selected().empty())
		{
			auto selected_item = ui_.stations_listbox.selected().at(0);
			auto station_name = ui_.stations_listbox.at(0).at(selected_item.item).text(0);
			ui_.current_station_label.caption(ui_.stations_listbox.at(0).at(selected_item.item).text(0));
		}
	}
	break;
	case events::Event::UPDATE_SONG_LABEL:
	{
		ui_.current_song_label.caption(context.stream_manager.get_song_title());
	}
	break;
	}
}
