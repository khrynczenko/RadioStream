...
bool StationPlayer::set_station(const Station& station) {
    ...
    if (possible_error.has_value()) {
        notify(std::make_any<BassErrorCode>(possible_error.value()),
               radiostream::Event::StreamLoadingError);
        return false;
    }
...
void StationPlayer::check_if_song_title_has_changed() {
    ...
    if (stream_manager_.get_song_title() != current_song_title_) {
        ...
        notify(std::make_any<std::string>(current_song_title_),
               radiostream::Event::StationSongHasChanged);
    }
}

Station StationPlayer::get_station() const { return station_; }
